...
public class _DbOldTree<K, V> {
   ...
   * @throws IOException
   */
  public _DbOldTree(DbTreeIODelegator io, String name, int order)
    throws IOException {
  ...
  public _DbOldTree(DbTreeIODelegator io, String name, int order,
    ...
    if (io.hasStructure(name)) {
      ...
      this.root = readDbTreeNode(new DbInputBytes(io.readNode(name, rootId)));
   ...
   * @throws IOException
   */
  protected void add(DbTreeNode<K, V> node, K key, V value) throws IOException {
    int childIndex = findChildKey(node, key);
    if (node.leaf) {
   ...
   * node.<br/>
   * <br/>
   * A divisão transforma um node cheio em 2 nodes e o elemento do meio sobe um
   * nível mais próximo da raiz.<br/>
   * <br/>
   * O parâmetro 'root' irá receber o elemento do meio e o parâmetro 'node' será
   * dividido em 2 nodes das quais o elemento que subiu para o 'root' irá
   ...
   * 
   * @param root node que receberá o elemento do meio
   * @param node node a ser dividido em dois
   * @param index
   */
    ...
    if (node.leaf) {
      ...
      if (removed) {
        ...
        if (nodeIsNotEnough(child)) {
          DbTreeNode<K, V> nextChild = nodeChildBigger(node, childIndex);
          if (nextChild != null) {
  ...
  protected void removeFixFirst(DbTreeNode<K, V> node, DbTreeNode<K, V> child,
  ...
  protected void removeMerge(DbTreeNode<K, V> node, DbTreeNode<K, V> child,
    ...
    DbTreeNode<K, V> leftNode =
      childIndex < brotherChildIndex ? child : childBrother;
    DbTreeNode<K, V> rightNode =
      childIndex < brotherChildIndex ? childBrother : child;
    int index = childIndex < brotherChildIndex ? childIndex : brotherChildIndex;
    removeMerge(node, index, leftNode, rightNode);
  }

  protected void removeMerge(DbTreeNode<K, V> root, int keyIndex,
  ...
  protected DbTreeNode<K, V> nodeChildBigger(DbTreeNode<K, V> node,
    ...
    DbTreeNode<K, V> nextChild =
      leftChildLength > rightChildLength ? leftChild : rightChild;
  ...
  protected DbTreeNode<K, V> nodeChildLeft(DbTreeNode<K, V> node, int childIndex)
    throws IOException {
  ...
  protected void nodeShiftKeyRight(DbTreeNode<K, V> node, int offset, int count) {
  ...
  protected void nodeAssignKey(DbTreeNode<K, V> node, int index, K key, V value) {
    node.keys[index] = key;
   ...
   * @param childNode
   */
  protected void insertNodeIdx(DbTreeNode<K, V> node, int idx, K key, V value,
    long childId, DbTreeNode<K, V> childNode) {
   ...
   * @param childIndex
   */
  protected void removeNodeSetIdx(DbTreeNode<K, V> node, int idx, int childIndex) {
  ...
  public static interface DbTreeLogger<K, V> {

    public void readChild(DbTreeNode<K, V> node, int childIndex, int childId);
  ...
  public static interface DbTreeIODelegator {
     ...
     * 
     * @param name
     * @param id código do node
     * @param bytes bytes do node
     * @throws IOException
     */
